...
const TranslatedMoviePlayer = ({ movie }) => {
  ...
  const startTracking = async () => {
    if (movie?.id && movie?.title && userIp && !isTracking) {
      await startMovieTracking(
        movie.id.toString(),
        movie.title,
        movie.poster,
        null, // userId can be null if not available
        userIp,
        'translated' // Use 'translated' as playerType
      );
      setIsTracking(true);
    }
  };
  ...
  useEffect(() => {
    ...
    // Add tracking to video events
    const handlePlay = () => {
      startTracking(); // Start tracking when video plays
    };
  ...
  }, [isPlaying, isMuted, isFullscreen, userIp, movie]);
