  ...
  return (
    <div className="w-full min-[1125px]:max-w-[24rem]">
      ...
      <div className="w-full flex flex-col gap-3 max-[1125px]:grid max-[1125px]:grid-cols-[repeat(auto-fit,minmax(306px,1fr))]">
        {recommendation?.slice(0, 7)?.map((item, index) => <Fragment key={index}><RecommendationCard data={item} type={!recommendation?.results?.length <= 5 ? item?.media_type : type} /></Fragment>)}
      </div>
    </div>
  )
}
