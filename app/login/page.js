...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      // Ban message is already shown by the authentication function
      // You can add additional handling here if needed
      console.log('User is banned:', result.banInfo);
    }
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-[#1e1c2f]/90 backdrop-blur-md border border-[#39374b] rounded-2xl shadow-2xl p-8">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white/10 hover:bg-white/20 border border-white/20 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-300 mb-6 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full bg-[#2d2a44] border border-[#39374b] rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 px-4 rounded-xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
              {loading ? (
