...
const MobileBottomNav = () => {
  ...
  const navItems = [
    ...
    { 
      name: 'Trending', 
      path: '/trending', // Changed from '/catalog?sort=TRENDING_DESC'
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className="flex justify-around items-center h-16">
      {navItems.map((item) => {
        const isActive = pathname === item.path || 
          (item.path === '/trending' && pathname.includes('/trending')) || // Updated
          (item.path === '/catalog' && pathname.includes('/catalog')) ||
          (item.path === '/news' && pathname.includes('/news')) ||
          (item.path === '/more' && pathname.includes('/more'))
