...
// Local cn function since the import path doesn't exist
function cn(...classes) {
  return classes.filter(Boolean).join(' ');
}

const ThreeDMarquee = ({ images, className, aspect, ...props }) => {
          ...
          >
            {chunks.map((subarray, colIndex) => (
              <motion.div
                ...
                animate={{ y: [0, -1000] }} // Continuous upward scroll
                transition={{
                  repeat: Infinity,
                  repeatType: "loop",
                  duration: 20, // Same speed as Cinextma
                  ease: "linear",
                }}
              >
                ...
                {subarray.map((image, imageIndex) => (
                  <div className="relative" key={`dup-${imageIndex + image}`}>
                    <GridLineHorizontal className="-top-4" offset="20px" />
                    <motion.img
                      src={image}
                      alt={`Image ${imageIndex + 1}`}
