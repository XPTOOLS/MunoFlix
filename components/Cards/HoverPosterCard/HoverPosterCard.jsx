...
const HoverPosterCard = ({ movie, fullWidth = false }) => {
  ...
  // Get image URLs with better fallback
  const getImageUrl = (path, size = 'w500') => {
    if (!path || imageError) {
      // Use a solid color or data URL as fallback
      return `data:image/svg+xml;base64,${btoa(`
        <svg width="400" height="225" xmlns="http://www.w3.org/2000/svg">
          <rect width="100%" height="100%" fill="#2d2a44"/>
          <text x="50%" y="50%" font-family="Arial" font-size="16" fill="#8884b8" text-anchor="middle" dy=".3em">${movieTitle}</text>
        </svg>
      `)}`;
    }
  ...
  // Check if movie is translated (safe check for both string and number IDs)
  const isTranslatedMovie = () => {
    if (typeof id === 'string') {
      return id.includes('translated-');
    }
    return false; // TMDB movies have number IDs
  };

  // Get the correct watch URL
  const getWatchUrl = () => {
    if (isTranslatedMovie()) {
      return `/watch/translated/${id}`;
    } else {
      return `/watch/${id}?media_type=${type}`;
    }
  };
